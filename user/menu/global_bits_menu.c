...
void global_bits_menu(uint16_t key) {
	...
	for(int i=0;i<CLUST_BIT_CNT;i++) {
		if(i>=first_visible_record && (i<(first_visible_record+max_records_cnt))) {
			...
			num_buf[0] = 'B';
			num_buf[1] = (i+1)/100 + '0';
			num_buf[2] = ((i+1)%100)/10 + '0';
			num_buf[3] = ((i+1)%10) + '0';
			bt816_cmd_text(300, 95+(i-first_visible_record)*30, 1, 0, num_buf);

			bt816_cmd_number(400, 95+(i-first_visible_record)*30, 1, 0, cl.cluster_bits[i]);
		}
	...
	if(key!=prev_key) {
		switch(key) {
			case KEY_LEFT:
				...
				break;
			case KEY_UP:
				if(first_visible_record>=max_records_cnt) first_visible_record-=max_records_cnt;
